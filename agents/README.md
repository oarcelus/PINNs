...
- `PINN.forward` calculates `u(x)`, differentiates it with respect to every input, calls every derivative except the last one `ux`, and calls the derivative of the last input `ut`. It feeds `[x, u, ux, ut]` to the dynamics MLP. With 17 inputs, the expected shapes are `x=[B,17]`, `u=[B,1]`, `ux=[B,16]`, `ut=[B,1]`, and dynamics input `[B,35]`; `cfg/pinn_config.json` currently matches that contract.
- There is a material scientific ambiguity: the loader places `Cycle Number` first, but the model interprets the final feature derivative as `ut`. No role may silently resolve that ambiguity by reordering columns or reinterpreting the PDE. The Architect must obtain or record an explicit decision.
