...
- **Scientific semantics:** What input is the independent/time coordinate; whether `ut`, `ux`, target, PDE residual, monotonicity direction, reductions, alpha/beta weights, and claimed generalization match the approved design. The current contract is `Cycle Number` at input index 0 as `t`, with `ut = du_dxt[:, 0:1]` and `ux = du_dxt[:, 1:]`; require a versioned specification for any departure. Flag random-row, adjacent-pair, group/cell, or temporal leakage.
