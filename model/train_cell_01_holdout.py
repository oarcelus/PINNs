...
history_csv_path = os.path.join(history_dir, "cell01_holdout_history.csv")
...
train_conditions = [
    ...
    "AG_25_80_A_03_features_cleaned_normalized__pchip.csv",
...
def run_one_epoch(pinn, loader, optimizer_u, optimizer_f, mse, relu, alpha, beta, device):
    pinn.train()
...
if __name__ == "__main__":
    ...
    logger = setup_logger(
        name="cell01_holdout",
        log_path=os.path.join(log_dir, "cell01_holdout.log"),
    )

    train_dat = FeatureDataset(path=data_path, conditions=train_conditions)
    test_dat = FeatureDataset(path=data_path, conditions=test_conditions)

    x1_train, x2_train, y1_train, y2_train = train_dat.load_all_cells()
    x1_test, x2_test, y1_test, y2_test = test_dat.load_all_cells()

    train_dataset = TensorDataset(
        torch.as_tensor(x1_train),
        torch.as_tensor(x2_train),
        torch.as_tensor(y1_train),
        torch.as_tensor(y2_train),
    )
    ...
    for iepoch in range(n_epochs):
        ...
        test_metrics = evaluate_loss(
            pinn, test_loader, mse, relu, alpha, beta, device
        )
        ...
        if test_metrics["loss"] < best_test_loss:
            ...
            if save_best_epoch:
                torch.save(
                    ...
                    os.path.join(save_dir, "cell01_holdout_best.pt"),
                )
    ...
    if save_last_epoch:
        torch.save(
            ...
            os.path.join(save_dir, "cell01_holdout_last.pt"),
        )
